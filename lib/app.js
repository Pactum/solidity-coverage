...
class App {
  constructor(config) {
    ...
    // Config
    this.config = config || {};
    this.workingDir = config.dir || '.';         // Relative path to project root directorh
    this.subDir =  config.sub || '';             // Relative path to contracts sub directory
  ...
  */
   ...
   */
  instrumentTarget() {
    this.skipFiles = this.skipFiles.map(contract => `${this.coverageDir}${this.subDir}/contracts/${contract}`);
    this.skipFiles.push(`${this.coverageDir}${this.subDir}/contracts/Migrations.sol`);

    let currentFile;
    try {
      shell.ls(`${this.coverageDir}${this.subDir}/contracts/**/*.sol`).forEach(file => {
  ...
  generateReport() {
    ...
    return new Promise((resolve, reject) => {
      try {
        this.coverage.generate(this.events, `${this.workingDir}${this.subDir}/contracts`);
